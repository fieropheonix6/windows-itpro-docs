...
- Authentication policy (AuthPolicy) - Indicates what type of authentication is required. For the MDM server, on-premises is the supported value, which means that the user is authenticated when calling the management service URL. This field is mandatory.
...
>
> - CertificateStore/WSTEP enables certificate renewal. If the server doesn't support it, don't set it.
